...
static const CGFloat CSToastFadeDuration        = 0.4;
...
-(void)makeCenterToast:(NSString *)message{
    CGPoint toastPoint = CGPointMake(ViewWidth/2, ViewHeight/2);
    NSValue * pointV = [NSValue valueWithCGPoint:toastPoint];
    
    [self makeToast:message duration:1.7 position:pointV];
}
...
- (void)makeCenterToast:(NSString *)message duration:(CGFloat)interval{
    ...
    [self makeToast:message duration:interval position:pointV];
                     ...
                     animations:^{
                         ...
                         [UIView animateWithDuration:CSToastFadeDuration
                                          ...
                                          animations:^{
                                              ...
                                              for (UIView *a in toast.subviews) {
                                                  l=(UILabel *)a;
                                                  
                                                  
                                              }
                                              
                                              toast.frame=CGRectMake(toast.center.x, toast.center.y, 0, 0);
                                              [self performSelector:@selector(sss:) withObject:l afterDelay:interval];
                                              toast.alpha = 0.0;
                                          } completion:^(BOOL finished) {
...
-(void)sss:(UILabel *)l{
    l.frame=CGRectMake(l.center.x, l.center.y, 0, 0);
}
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (CSToastDisplayShadow) {
        wrapperView.layer.shadowColor = black74TitleColor.CGColor;
        wrapperView.layer.shadowOpacity = CSToastShadowOpacity;
        wrapperView.layer.shadowRadius = CSToastShadowRadius;
        wrapperView.layer.shadowOffset = CSToastShadowOffset;
    }
    
    wrapperView.backgroundColor = [black74TitleColor colorWithAlphaComponent:CSToastOpacity];
