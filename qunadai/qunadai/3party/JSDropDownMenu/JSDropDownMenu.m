...
// 选中颜色加深
#define SelectColor [UIColor colorWithRed:255.0f/255.0f green:255.0f/255.0f blue:255.0f/255.0f alpha:1.0]
...
- (void)setDataSource:(id<JSDropDownMenuDataSource>)dataSource {
    ...
    leftlabel.text = @"筛选：";
    leftlabel.textColor = blackTitleColor;
    ...
    CGFloat textLayerInterval = (self.frame.size.width -65)/ ( _numOfMenu * 2);
    
    CGFloat separatorLineInterval = (self.frame.size.width-65) / _numOfMenu;
    
    CGFloat bgLayerInterval = (self.frame.size.width-65)/ _numOfMenu;
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        //bgLayer
        CGPoint bgLayerPosition = CGPointMake((i+0.5)*bgLayerInterval+65, self.height/2);
        CALayer *bgLayer = [self createBgLayerWithColor:[UIColor clearColor] andPosition:bgLayerPosition];
        [self.layer addSublayer:bgLayer];
        [tempBgLayers addObject:bgLayer];
        //title
        CGPoint titlePosition = CGPointMake( (i * 2 + 1) * textLayerInterval+60 , self.frame.size.height / 2);
        NSString *titleString = [_dataSource menu:self titleForColumn:i];
        CATextLayer *title = [self createTextLayerWithNSString:titleString withColor:self.textColor andPosition:titlePosition];
        [self.layer addSublayer:title];
        [tempTitles addObject:title];
        //indicator
        CAShapeLayer *indicator = [self createIndicatorWithColor:self.indicatorColor andPosition:CGPointMake(CGRectGetMaxX(title.frame) + 12, self.frame.size.height / 2+1)];
        [self.layer addSublayer:indicator];
        [tempIndicators addObject:indicator];
        
        //separator
         CGPoint separatorPosition = CGPointMake((i ) * separatorLineInterval+65, self.frame.size.height/2+1);
...
- (CALayer *)createBgLayerWithColor:(UIColor *)color andPosition:(CGPoint)position {
    ...
    layer.bounds = CGRectMake(0, 0, (self.frame.size.width-65)/self.numOfMenu, self.frame.size.height-1);
...
- (CAShapeLayer *)createSeparatorLineWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    [path addLineToPoint:CGPointMake(0, self.frame.size.height-16)];
...
- (CATextLayer *)createTextLayerWithNSString:(NSString *)string withColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    CGFloat sizeWidth = (size.width < ((self.frame.size.width-65) / _numOfMenu) - 25) ? size.width : (self.frame.size.width-65) / _numOfMenu - 25;
    layer.bounds = CGRectMake(0, 0, sizeWidth, size.height);
    layer.string = string;
    
    layer.font = (__bridge CFTypeRef _Nullable)(@".PingFang-SC-Light");
    layer.fontSize = 13.0;
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    tapIndex = (touchPoint.x-65) / ((self.frame.size.width-65) / _numOfMenu);
    ...
    if (displayByCollectionView) {
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            [(CALayer *)self.bgLayers[tapIndex] setBackgroundColor:[UIColor clearColor].CGColor];
        } else {
            _currentSelectedMenudIndex = tapIndex;
...
//        UITableView *leftTableView = _leftTableView;
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            if (_leftSelectedRow!=0) {
                [(CATextLayer*)self.titles[tapIndex] setForegroundColor:ThemeColor.CGColor];
            }else{
                [(CATextLayer*)self.titles[tapIndex] setForegroundColor:[UIColor colorWithRed:102.0/255.0 green:102.0/255.0 blue:102.0/255.0 alpha:1.0].CGColor];
            }
        }
    }
}
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        ...
        // 只取宽度
        textSize = [titleLabel.text textSizeWithFont:QNDFont(14.0) constrainedToSize:CGSizeMake(MAXFLOAT, 14) lineBreakMode:NSLineBreakByWordWrapping];
    ...
    if (leftOrRight == 1) {
        ...
        titleLabel.frame = CGRectMake(marginX+16+15, 0, textSize.width, cell.frame.size.height);
        //右边tableview
        
        if ([titleLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
            
            UIImageView *accessoryImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"pick_xuanzhong"]];
            
            accessoryImageView.frame = CGRectMake(marginX, (self.frame.size.height-12)/2, 16, 12);
...
- (void)confiMenuWithSelectRow:(NSInteger)row leftOrRight:(NSInteger)leftOrRight{
    ...
    [(CALayer *)self.bgLayers[_currentSelectedMenudIndex] setBackgroundColor:[UIColor clearColor].CGColor];
