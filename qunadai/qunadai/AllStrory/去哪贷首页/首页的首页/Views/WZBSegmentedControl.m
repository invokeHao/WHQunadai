...
- (void)setupBackgroundView {
    ...
    UIView * sliderView = [[UIView alloc]initWithFrame:CGRectMake(self.frame.size.width / self.titles.count/4, 0, self.frame.size.width / self.titles.count/2, 5)];
    [self.backgroundView addSubview:sliderView];
...
- (void)setupAllButton {
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        button.titleLabel.font = QNDFont(14.0);
...
-(void)setTheUnapplicationNum:(NSInteger)num1 andApplicationNum:(NSInteger)num2{
    UIButton * unApplicationBtn = [self viewWithTag:WZBButtonTag+2];
    UIButton * applicationBtn = [self viewWithTag:WZBButtonTag+3];
    NSString * unapplicationStr = FORMAT(@"未申请(%ld)",num1);
    NSString * applicationStr = FORMAT(@"已申请(%ld)",num2);

    [unApplicationBtn setTitle:unapplicationStr forState:UIControlStateNormal];
    [applicationBtn setTitle:applicationStr forState:UIControlStateNormal];
}
